<template>
  <div>
    ...
    <!-- 末日生存：六维属性 -->
    <div class="border border-accent/20 rounded-xs p-2 mb-3">
      <p class="text-xs text-accent mb-1.5">属性</p>
      <div class="grid grid-cols-3 gap-1 text-xs">
        <div
          v-for="attr in attributeList"
          :key="attr.key"
          class="flex justify-between border border-accent/10 rounded-xs px-1.5 py-0.5"
        >
          <span class="text-muted">{{ attr.name }}</span>
          <span
            >{{ playerStore.attributes[attr.key] }} ({{
              playerStore.getModifier(attr.key) >= 0 ? "+" : ""
            }}{{ playerStore.getModifier(attr.key) }})</span
          >
        </div>
      </div>
      <p class="text-[10px] text-muted mt-1">
        派生：战斗
        {{ playerStore.getDerivedSkillModifier("combat") >= 0 ? "+" : ""
        }}{{ playerStore.getDerivedSkillModifier("combat") }} · 制作
        {{ playerStore.getDerivedSkillModifier("craft") >= 0 ? "+" : ""
        }}{{ playerStore.getDerivedSkillModifier("craft") }} · 搜寻
        {{ playerStore.getDerivedSkillModifier("scavenge") >= 0 ? "+" : ""
        }}{{ playerStore.getDerivedSkillModifier("scavenge") }}
      </p>
    </div>
...
import { ATTRIBUTE_NAMES } from "@/types/attributes";
import type { AttributeType } from "@/types";

const playerStore = usePlayerStore();

const attributeList = computed(() =>
  (["str", "dex", "con", "int", "wis", "cha"] as AttributeType[]).map(
    (key) => ({ key, name: ATTRIBUTE_NAMES[key] }),
  ),
);
