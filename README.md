...
**技能成长** — 农耕、采集、钓鱼、挖矿、战斗、烹饪、锻造七大技能，Lv5/10/15/20 可选择专精方向（锻造最高 20 级）
...
**装备与锻造** — 武器/帽/鞋/戒 + 25 套套装效果；铁匠铺当场锻造（节奏小游戏、四档品质、词条重刷）；图纸多来源解锁；背包/矿洞一键穿戴整套
...
- **禁止**在业务代码中直接使用 `crypto.randomUUID()`（生产 HTTP 环境下会失败）。请使用 `src/utils/id.ts` 的 `createId()`。详见 [AGENT.md](./AGENT.md) §2.4。
...
| 工具     | 水壶/锄头/镐/鱼竿等，当场升级（基础→铁/精钢→铱金），需锻造等级，降低体力消耗      |
| 锻造     | 铁匠铺工坊：配方打造、图纸店、请教、任务板、词条重刷、工具升级                     |
| 装备     | 武器、帽子、鞋子、戒指，25 套套装；打造品四档品质 + 词条                           |
